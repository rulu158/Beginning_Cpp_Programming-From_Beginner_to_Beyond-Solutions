...
void c_math_functions() {
    ...
    // to round up to the nearest cent, we first select how many precision we need by
    // multiplying b 100 (two decimals) so it will take the integer part with those
    // two decimals and round up (ceil) to the nearest upper integer, and then add
    // those two decimals again by dividing by the same number (100 -> two decimals)
